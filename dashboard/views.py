...
def dashboard(request):
    ...
    context = {
        ...
        # Localização de referência (sede)
        'reference_location': {
            'lat': -23.5048633,
            'lng': -46.8532187,
...
def tecnicos_mapa_api(request):
    ...
    # Buscar todos os técnicos ativos (inclui os sem coordenadas)
    tecnicos = Tecnico.objects.filter(ativo=True)

    data = []
    for tecnico in tecnicos:
        # Se não tiver lat/lng, tentar geocodificar a partir do endereço do técnico
        lat = tecnico.latitude
        lng = tecnico.longitude
        if (not lat or not lng) and tecnico.endereco:
            ...
            try:
                ...
                if result:
                    lat, lng = result
                    # Persistir coordenadas no modelo (silenciosamente)
                    try:
                        tecnico.latitude = lat
                        tecnico.longitude = lng
                        tecnico.save(update_fields=['latitude', 'longitude'])
                    except Exception:
                        # Ignore save errors to avoid breaking the API
                        pass
            except Exception:
                lat = lat or None
                lng = lng or None
        ...
        data.append({
            ...
            'status_display': tecnico.get_status_display(),
