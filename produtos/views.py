...
@require_POST
def criar_categoria_produto_ajax(request):
    """
    Cria uma nova categoria para produtos via AJAX e retorna JSON.
    """
...
def criar_unidade_ajax(request):
    """
    Cria uma nova unidade via AJAX e retorna JSON.
    """
    if not request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return JsonResponse({'success': False, 'message': 'Requisição inválida'})
    ...
    if not sigla or not nome:
        return JsonResponse({'success': False, 'message': 'Sigla e nome são obrigatórios'})

    if UnidadeMedida.objects.filter(sigla__iexact=sigla).exists():
        return JsonResponse({'success': False, 'message': 'Já existe uma unidade com esta sigla'})

    try:
        unidade = UnidadeMedida.objects.create(sigla=sigla, nome=nome)
        return JsonResponse({
            'success': True,
        ...
        return JsonResponse({'success': False, 'message': f'Erro ao criar unidade: {str(e)}'})