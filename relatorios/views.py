...
def relatorio_financeiro(request):
    ...
    # --- Paginação simples para o resumo por mês ---
    # Parâmetros via GET: rows (itens por página), page (1-based)
    try:
    ...
    total_items = len(dados_ultimos_6_meses)
    total_pages = (total_items + rows - 1) // rows if rows else 1

    # Ajustar página se exceder
    if page > total_pages:
        page = total_pages

    start = (page - 1) * rows
    end = start + rows
    dados_pagina = dados_ultimos_6_meses[start:end]
    ...
    context = {
        'faturamento_mensal': faturamento_mensal,
        'faturamento_anual': faturamento_anual,
        # For template rendering (loop) provide the Python list
        'dados_ultimos_6_meses_list': dados_pagina,
        # For JS, provide a JSON string
        'dados_ultimos_6_meses': json.dumps(dados_pagina),
        'titulo': 'Relatório Financeiro',
        'variacao_percentual': variacao_percentual,
        'top_clientes': top_clientes,
        'top_tecnicos': top_tecnicos,
    ...
    # JSON string version for safe insertion into JS
    context['pagination_json'] = json.dumps(context['pagination'])
