...
from utils.geocode import geocode_address, geocode_address_variants
        ...
        try:
            ...
            with transaction.atomic():
                ...
                # Salvar o técnico
                    ...
                    # Tentar geocodificar imediatamente após criação (usar CEP/address quando disponível)
                    try:
                        endereco_para_geocode = None
                        # Preferir CEP formatado se houver
                        if tecnico.cep:
                            endereco_para_geocode = f"{tecnico.endereco} {tecnico.numero or ''} {tecnico.bairro or ''} {tecnico.cidade or ''} {tecnico.estado or ''} {tecnico.cep}"
                        else:
                            endereco_para_geocode = f"{tecnico.endereco} {tecnico.numero or ''} {tecnico.bairro or ''} {tecnico.cidade or ''} {tecnico.estado or ''}"

                        result_coords = None
                        try:
                                result_coords = geocode_address_variants(
                                    address=endereco_para_geocode,
                                    cep=tecnico.cep,
                        ...
                        if result_coords:
                            ...
                            try:
                                ...
                                # informar via mensagem curta
                                messages.info(request, 'Coordenadas obtidas e pino do técnico adicionado ao mapa.')
                            except Exception:
                                # salvamento falhou — não interromper o fluxo principal
                                pass
                        else:
                            # Não conseguiu geocodificar agora; o mapa tentará novamente ao carregar
                            messages.info(request, 'Técnico criado, mas sem coordenadas automáticas. Você pode tentar gerar localização manualmente.')
                    except Exception:
                        # Não propagar erro de geocoding para o usuário
                        pass
...
def mapa_tecnicos(request):
    # Tentar geocodificar técnicos sem coordenadas a partir do endereço
    tecnicos_sem_coordenadas = list(Tecnico.objects.filter(
        models.Q(latitude__isnull=True) | models.Q(longitude__isnull=True)
    ).order_by('nome_completo'))

    for tecnico in tecnicos_sem_coordenadas:
        if (not tecnico.latitude or not tecnico.longitude) and tecnico.endereco:
            endereco = f"{tecnico.endereco} {tecnico.numero or ''} {tecnico.bairro or ''} {tecnico.cidade or ''} {tecnico.estado or ''}"
            try:
                result = geocode_address_variants(address=endereco, cep=tecnico.cep, cidade=tecnico.cidade, estado=tecnico.estado)
                if result:
                    lat, lng = result
                    tecnico.latitude = lat
                    tecnico.longitude = lng
                    try:
                        tecnico.save(update_fields=['latitude', 'longitude'])
                    except Exception:
                        pass
            except Exception:
                # Não bloquear renderização do mapa em caso de falha de geocoding
                pass

    # Buscar todos os técnicos que possuem coordenadas de geolocalização (recalcula após geocoding)
    ...
    context = {
        ...
        # JSON-serializado para uso no JS (garante formato numérico com ponto decimal)
        'tecnicos_json': json.dumps([
            {
                'id': t.id,
                ...
                'lat': float(t.latitude) if t.latitude is not None else None,
                'lng': float(t.longitude) if t.longitude is not None else None,
                'ultima_atualizacao': t.ultima_atualizacao_local.isoformat() if getattr(t, 'ultima_atualizacao_local', None) else None,
                'slug': t.slug,
        ...
        'reference_location': {
            'lat': -23.5048633,
            'lng': -46.8532187,
            'label': 'Sede - Alameda Xingu, 350 - Alphaville, Barueri - SP'
        }
    }
...
@login_required
def geocode_tecnico_api(request):
    """API POST para tentar geocodificar um técnico específico pelo seu ID."""
    if request.method != 'POST':
        return JsonResponse({'success': False, 'message': 'Método não permitido'}, status=405)

    tecnico_id = request.POST.get('tecnico_id') or request.GET.get('tecnico_id')
    if not tecnico_id:
        return JsonResponse({'success': False, 'message': 'ID do técnico não informado'}, status=400)

    try:
        tecnico = Tecnico.objects.get(id=int(tecnico_id))
    except Exception:
        return JsonResponse({'success': False, 'message': 'Técnico não encontrado'}, status=404)

    endereco = f"{tecnico.endereco} {tecnico.numero or ''} {tecnico.bairro or ''} {tecnico.cidade or ''} {tecnico.estado or ''} {tecnico.cep or ''}"
    try:
        result = geocode_address_variants(address=endereco, cep=tecnico.cep, cidade=tecnico.cidade, estado=tecnico.estado)
        if result:
            lat, lng = result
            ...
            tecnico.ultima_atualizacao_local = timezone.now()
            tecnico.save(update_fields=['latitude', 'longitude', 'ultima_atualizacao_local'])
            return JsonResponse({'success': True, 'lat': lat, 'lng': lng, 'message': 'Coordenadas atualizadas'})
        else:
            return JsonResponse({'success': False, 'message': 'Não foi possível obter coordenadas para esse endereço'}, status=422)
    except Exception as e:
        return JsonResponse({'success': False, 'message': 'Erro ao geocodificar', 'detail': str(e)}, status=500)
