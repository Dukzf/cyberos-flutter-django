...
def perfil_detalhe(request):
    """Exibe a página de detalhes do perfil do usuário atual."""
    user = request.user
    try:
        perfil = user.perfil
    except Perfil.DoesNotExist:
        perfil = Perfil.objects.create(user=user, tipo='tecnico')

    # Handle profile photo upload by the user themselves
    if request.method == 'POST' and request.user == user:
        # We expect a field named 'foto' in FILES
        foto = request.FILES.get('foto')
        if foto:
            perfil.foto = foto
            perfil.save()
            messages.success(request, 'Foto de perfil atualizada com sucesso.')
            return redirect('usuarios:perfil_detalhe')
        else:
...
def configurar_permissoes_usuario(request, usuario_id):
    usuario = get_object_or_404(User, id=usuario_id)
    # Garantir que o usuário tenha um perfil; caso contrário, criar um perfil padrão
    try:
        perfil = usuario.perfil
    except Perfil.DoesNotExist:
        perfil = Perfil.objects.create(user=usuario, tipo='tecnico')
        # Opcional: configurar permissões padrão para este novo perfil
        configurar_permissoes_padrao(perfil)
